...
A single segmentation mask from SAM2.
...
| `segmentation_masks` | list[SegmentationMask] \| null | Segmentation masks from SAM2 |
| `vla_action` | list[float] \| null | VLA action vector for robotic control |
...
| `prediction` | PredictionResult \| null | Raw prediction result |
| `observation` | Observation \| null | Processed observation |
| `reasoning` | AgentReasoning \| null | LLM reasoning output |
...
| `time_range` | str \| null | Time filter (e.g. "last_1h", "last_24h") |
