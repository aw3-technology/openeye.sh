...
class KnowledgeBase:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        embedding_type : str
            Embedding backend: "http" (default) for local server, "nebius" for
            Nebius Token Factory Qwen3-Embedding.  When "nebius", pass ``api_key``
            and optionally ``embedding_model`` / ``embedding_base_url`` in kwargs.
        **kwargs
            Additional keyword arguments.  Recognised keys for ``embedding_type="nebius"``:
            ``api_key``, ``embedding_model``, ``embedding_base_url``.
        """
        ...
        if retriever_type == "faiss":
            if embedding_type == "nebius":
                from .nebius_embedding import NebiusEmbeddingClient

                self.embedding_client = NebiusEmbeddingClient(
                    api_key=kwargs["api_key"],
                    model=kwargs.get("embedding_model", "Qwen/Qwen3-Embedding-8B"),
