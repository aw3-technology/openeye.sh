"""Nebius Token Factory VLM provider — processes camera frames via Nebius vision API."""
...
@singleton
class VLMNebiusProvider:
    def __init__(
        self,
        base_url: str,
        api_key: str,
        model: str = "Qwen/Qwen3-VL-72B",
        fps: int = 10,
        camera_index: int = 0,
    ):
    ...
    async def _process_frame(self, frame: str):
        ...
        try:
            ...
            logging.debug(f"Nebius VLM processing latency: {processing_latency:.3f}s")
            if self.message_callback:
                self.message_callback(response)
        except Exception as e:
            logging.error(f"Nebius VLM error processing frame: {e}")
    ...
    def start(self):
        ...
        logging.info("Nebius VLM provider started")
