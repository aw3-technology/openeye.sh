...
import { priorityColor, priorityBadge } from "./constants";
...
export function ActionPlanCard({ frame, running }: ActionPlanCardProps) {
  return (
    <Card className="border-terminal-green/20">
        ...
        <CardTitle className="text-sm font-mono flex items-center gap-2 text-terminal-green">
          ...
          {frame && (
            <Badge variant="outline" className="ml-auto text-[10px] border-terminal-green/30">
              {frame.action_plan.length} steps
            </Badge>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <AnimatePresence mode="popLayout">
          {frame?.action_plan && frame.action_plan.length > 0 ? (
