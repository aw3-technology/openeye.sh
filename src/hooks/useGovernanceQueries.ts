...
export function useEnablePolicy() {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(`Failed to enable policy: ${String(err)}`),
    onSettled: () => {
...
export function useDisablePolicy() {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(`Failed to disable policy: ${String(err)}`),
    onSettled: () => {
      qc.invalidateQueries({ queryKey: ["governance", "policies"] });
      qc.invalidateQueries({ queryKey: ["governance", "status"] });
    },
...
export function useLoadPreset() {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(`Failed to load preset: ${String(err)}`),
    onSettled: () => {
...
export function useUpdateGovernanceConfig() {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(`Invalid config: ${String(err)}`),
    onSettled: () => {
      qc.invalidateQueries({ queryKey: ["governance"] });
    },
  });
